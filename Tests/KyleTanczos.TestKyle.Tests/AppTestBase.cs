...
using KyleTanczos.TestKyle.Tests.TestDatas;
...
namespace KyleTanczos.TestKyle.Tests
{
    ...
    /// Provides methods to easily work with <see cref="TestKyleDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase
    {
        ...
        protected override void AddModules(ITypeList<AbpModule> modules)
        {
            ...
            modules.Add<TestKyleTestModule>();
        }

        #region UsingDbContext

        protected void UsingDbContext(Action<TestKyleDbContext> action)
        {
            using (var context = LocalIocManager.Resolve<TestKyleDbContext>())
            {
        ...
        protected async Task UsingDbContextAsync(Action<TestKyleDbContext> action)
        {
            using (var context = LocalIocManager.Resolve<TestKyleDbContext>())
            {
                context.DisableAllFilters();
        ...
        protected T UsingDbContext<T>(Func<TestKyleDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<TestKyleDbContext, Task<T>> func)
        {
            T result;
